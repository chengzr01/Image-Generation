...
class BatchNorm1d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=None, is_train=True, sync=True):
        ...
        self.running_mean = init.constant(
            (num_features,), "float32", 0.0).stop_grad()
        self.running_var = init.constant(
            (num_features,), "float32", 1.0).stop_grad()
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        def block(in_feat, out_feat, normalize=True):
            ...
            return layers
        self.model = nn.Sequential(*block(opt.latent_dim, 128, normalize=False), *block(128, 256), *block(
            256, 512), *block(512, 1024), nn.Linear(1024, int(np.prod(img_shape))), nn.Tanh())
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear(int(np.prod(img_shape)), 512), nn.LeakyReLU(
...
dataloader = MNIST(train=True, transform=transform).set_attrs(
